...
BEGIN
  ...
  IF NEW.penalty_type = 'days' THEN
    NEW.penalty_amount := ROUND((COALESCE(NEW.basic_salary, 0) / 30.0) * normalized_penalty_value * 8) / 8.0;
  ...
  NEW.leave_deduction := ROUND((base_gross / 30.0) * normalized_leave_days * 8) / 8.0;
