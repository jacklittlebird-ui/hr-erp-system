...
BEGIN
  normalized_leave_days := ROUND(COALESCE(NEW.leave_days, 0) * 4) / 4.0;
  NEW.leave_days := normalized_leave_days;

  normalized_penalty_value := CASE
    WHEN NEW.penalty_type = 'days' THEN ROUND(COALESCE(NEW.penalty_value, 0) * 4) / 4.0
  ...
  ELSIF NEW.penalty_type = 'percentage' THEN
    NEW.penalty_amount := ROUND(COALESCE(NEW.basic_salary, 0) * normalized_penalty_value / 100, 2);
  ELSE
    NEW.penalty_amount := normalized_penalty_value;
  END IF;
...
SET
  leave_days = ROUND(COALESCE(leave_days, 0) * 4) / 4.0,
  penalty_value = CASE
    WHEN penalty_type = 'days' THEN ROUND(COALESCE(penalty_value, 0) * 4) / 4.0
...
WHERE leave_days IS NOT NULL OR penalty_value IS NOT NULL;